...
class LDA(object):
	...
	def __init__(self,docs,alpha,beta,K,num_iterations=10):
		# Assumes documents are tokenized, stopword filtered etc. Each document is a list of tokens.
		self.docs = docs
		...
		# Document - vocab count. Keep track of how much number of words are there in each document	
	...
	def _setup(self):
		# Set up word-document associations and document word associations prior to running inference. 
		words = OrderedDict()		
		...
		for it in range(self.n_iter):
			...
			for i in range(len(self.z)):
				...
				for k in range(self.K):
					# Update equation.
					term_1 = (self.n_k_w[w,k] + self.beta) / (self.n_k[k] + self.W*self.beta)
					word = self.w_z_index[i]				
					# Number of words in the document except this word														
					n_d_i = self.d_w_count[d] -  self.d_w_mapping[d].get(word,0)														
					term_2 = (self.n_d_k[d,k] + self.alpha) / (n_d_i + self.alpha*self.K)
